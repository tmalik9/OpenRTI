...
main(int argc, char* argv[])
{
  ...
  std::unique_ptr<rti1516e::LogicalTimeFactory> factory;
  ...
  std::unique_ptr<rti1516e::LogicalTimeInterval> logicalTimeInterval = factory->makeZero();
  ...
  std::unique_ptr<rti1516e::LogicalTime> logicalTime = factory->makeInitial();
  if (!logicalTime.get()) {
  ...
  std::unique_ptr<rti1516e::LogicalTime> initialTime = factory->makeInitial();
  if (!initialTime.get()) {
    std::cerr << "Can not create logical time: \"" << implementationName << "\"!" << std::endl;
    return EXIT_FAILURE;
  }

  std::unique_ptr<rti1516e::LogicalTime> finalTime = factory->makeFinal();
  if (!finalTime.get()) {
    std::cerr << "Can not create logical time: \"" << implementationName << "\"!" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  std::unique_ptr<rti1516e::LogicalTime> logicalTime2 = factory->makeInitial();
  if (!logicalTime2.get()) {
    std::cerr << "Can not create logical time: \"" << implementationName << "\"!" << std::endl;
    return EXIT_FAILURE;
  }
  std::unique_ptr<rti1516e::LogicalTimeInterval> logicalTimeInterval2 = factory->makeZero();
