...
void MeasurementInitInteractionClass::sendWithTime(const std::vector<uint8_t>& Dummy, int64_t time)
{
  rti1516ev::ParameterHandleValueMap parameters;
  rti1516ev::HLAopaqueData DummyEncoder(Dummy);
  parameters.insert(std::make_pair(GetDummyParameterHandle(), DummyEncoder.encode()));
...
void MeasurementInitInteractionClass::ReceiveInteraction(const rti1516ev::ParameterHandleValueMap& parameters , const rti1516ev::LogicalTime& time, OrderType orderType)
{
  ...
  for (auto& entry : _receiveCallbacksWithTime) {
    auto& callback = entry.second;
    callback(DummyDecoder.get(), static_cast<const rti1516ev::HLAinteger64Time&>(time).getTime(), orderType);
  }
}
...
uint32_t MeasurementInitInteractionClass::RegisterReceiveCallbackWithTime(ReceiveCallbackWithTime callback)
{
  uint32_t key = _receiveCallbacksWithTimeNextKey++;
  _receiveCallbacksWithTime.insert(std::make_pair(key, callback));
  return key;
...
void MeasurementStopInteractionClass::ReceiveInteraction(const rti1516ev::ParameterHandleValueMap& parameters , const rti1516ev::LogicalTime& time, OrderType orderType)
{
  ...
  for (auto& entry : _receiveCallbacksWithTime) {
    auto& callback = entry.second;
    callback(NextFederationSuffixDecoder.get(), static_cast<const rti1516ev::HLAinteger64Time&>(time).getTime(), orderType);
  }
}
...
void KeyEventInteractionClass::ReceiveInteraction(const rti1516ev::ParameterHandleValueMap& parameters , const rti1516ev::LogicalTime& time, OrderType orderType)
{
  ...
  for (auto& entry : _receiveCallbacksWithTime) {
    auto& callback = entry.second;
    callback(KeyCodeDecoder.get(), static_cast<const rti1516ev::HLAinteger64Time&>(time).getTime(), orderType);
  }
}
...
void TextLogInteractionClass::ReceiveInteraction(const rti1516ev::ParameterHandleValueMap& parameters , const rti1516ev::LogicalTime& time, OrderType orderType)
{
  ...
  for (auto& entry : _receiveCallbacksWithTime) {
    auto& callback = entry.second;
    callback(SenderDecoder.get(), TextDecoder.get(), static_cast<const rti1516ev::HLAinteger64Time&>(time).getTime(), orderType);
  }
}
...
void DOMemberTransmitDataInteractionClass::ReceiveInteraction(const rti1516ev::ParameterHandleValueMap& parameters , const rti1516ev::LogicalTime& time, OrderType orderType)
{
  ...
  for (auto& entry : _receiveCallbacksWithTime) {
    auto& callback = entry.second;
    callback(static_cast<DOMemberSourceObjectClass*>(GetObjectClassRegistry()->GetDOMemberSourceObjectClass())->GetObjectInstance(ObjInstanceHandleDecoder.get()), ConnectionTypeDecoder.get(), DataBytesDecoder.get(), static_cast<const rti1516ev::HLAinteger64Time&>(time).getTime(), orderType);
  }
}
...
void SystemVariableUpdateInteractionClass::ReceiveInteraction(const rti1516ev::ParameterHandleValueMap& parameters , const rti1516ev::LogicalTime& time, OrderType orderType)
{
  ...
  for (auto& entry : _receiveCallbacksWithTime) {
    auto& callback = entry.second;
    callback(IdDecoder.get(), ValueDecoder.get(), ClientDecoder.get(), HasChangedDecoder.get(), static_cast<const rti1516ev::HLAinteger64Time&>(time).getTime(), orderType);
  }
}
...
void SystemVariableModificationInteractionClass::ReceiveInteraction(const rti1516ev::ParameterHandleValueMap& parameters , const rti1516ev::LogicalTime& time, OrderType orderType)
{
  ...
  for (auto& entry : _receiveCallbacksWithTime) {
    auto& callback = entry.second;
    callback(ValueDecoder.get(), static_cast<const rti1516ev::HLAinteger64Time&>(time).getTime(), orderType);
  }
}
...
void ValueEntityUpdateInteractionClass::ReceiveInteraction(const rti1516ev::ParameterHandleValueMap& parameters , const rti1516ev::LogicalTime& time, OrderType orderType)
{
  ...
  for (auto& entry : _receiveCallbacksWithTime) {
    auto& callback = entry.second;
    callback(IdDecoder.get(), ValueDecoder.get(), static_cast<const rti1516ev::HLAinteger64Time&>(time).getTime(), orderType);
  }
}
...
void BusMessageInteractionClass::ReceiveInteraction(const rti1516ev::ParameterHandleValueMap& parameters , const rti1516ev::LogicalTime& time, OrderType orderType)
{
  ...
  for (auto& entry : _receiveCallbacksWithTime) {
    auto& callback = entry.second;
    callback(IsRequestDecoder.get(), ChannelNameDecoder.get(), static_cast<BusType>(BusTypeDecoder.get()), RequestingFederateDecoder, SenderDecoder, ReceiverDecoder, static_cast<const rti1516ev::HLAinteger64Time&>(time).getTime(), orderType);
  }
}
...
void EthPacketInteractionClass::ReceiveInteraction(const rti1516ev::ParameterHandleValueMap& parameters , const rti1516ev::LogicalTime& time, OrderType orderType)
{
  ...
  for (auto& entry : _receiveCallbacksWithTime) {
    auto& callback = entry.second;
    callback(IsRequestDecoder.get(), ChannelNameDecoder.get(), static_cast<BusType>(BusTypeDecoder.get()), RequestingFederateDecoder, SenderDecoder, ReceiverDecoder, FrameDecoder, PortNameDecoder.get(), static_cast<const rti1516ev::HLAinteger64Time&>(time).getTime(), orderType);
  }
}
...
void EthPacketErrorInteractionClass::ReceiveInteraction(const rti1516ev::ParameterHandleValueMap& parameters , const rti1516ev::LogicalTime& time, OrderType orderType)
{
  ...
  for (auto& entry : _receiveCallbacksWithTime) {
    auto& callback = entry.second;
    callback(IsRequestDecoder.get(), ChannelNameDecoder.get(), static_cast<BusType>(BusTypeDecoder.get()), RequestingFederateDecoder, SenderDecoder, ReceiverDecoder, FrameDecoder, static_cast<const rti1516ev::HLAinteger64Time&>(time).getTime(), orderType);
  }
}
...
void CANMessageInteractionClass::ReceiveInteraction(const rti1516ev::ParameterHandleValueMap& parameters , const rti1516ev::LogicalTime& time, OrderType orderType)
{
  ...
  for (auto& entry : _receiveCallbacksWithTime) {
    auto& callback = entry.second;
    callback(IsRequestDecoder.get(), ChannelNameDecoder.get(), static_cast<BusType>(BusTypeDecoder.get()), RequestingFederateDecoder, SenderDecoder, ReceiverDecoder, IdDecoder.get(), FrameDecoder, static_cast<const rti1516ev::HLAinteger64Time&>(time).getTime(), orderType);
  }
}
...
void CANErrorFrameInteractionClass::ReceiveInteraction(const rti1516ev::ParameterHandleValueMap& parameters , const rti1516ev::LogicalTime& time, OrderType orderType)
{
  ...
  for (auto& entry : _receiveCallbacksWithTime) {
    auto& callback = entry.second;
    callback(IsRequestDecoder.get(), ChannelNameDecoder.get(), static_cast<BusType>(BusTypeDecoder.get()), RequestingFederateDecoder, SenderDecoder, ReceiverDecoder, FrameDecoder.get(), static_cast<const rti1516ev::HLAinteger64Time&>(time).getTime(), orderType);
  }
}
...
void FlexRaySymbolInteractionClass::ReceiveInteraction(const rti1516ev::ParameterHandleValueMap& parameters , const rti1516ev::LogicalTime& time, OrderType orderType)
{
  ...
  for (auto& entry : _receiveCallbacksWithTime) {
    auto& callback = entry.second;
    callback(IsRequestDecoder.get(), ChannelNameDecoder.get(), static_cast<BusType>(BusTypeDecoder.get()), RequestingFederateDecoder, SenderDecoder, ReceiverDecoder, static_cast<FlexRaySymbolPattern>(SymbolPatternDecoder.get()), static_cast<FlexRayChannel>(FlexRayChannelDecoder.get()), static_cast<const rti1516ev::HLAinteger64Time&>(time).getTime(), orderType);
  }
}
...
void FlexRayCycleStartInteractionClass::ReceiveInteraction(const rti1516ev::ParameterHandleValueMap& parameters , const rti1516ev::LogicalTime& time, OrderType orderType)
{
  ...
  for (auto& entry : _receiveCallbacksWithTime) {
    auto& callback = entry.second;
    callback(IsRequestDecoder.get(), ChannelNameDecoder.get(), static_cast<BusType>(BusTypeDecoder.get()), RequestingFederateDecoder, SenderDecoder, ReceiverDecoder, CycleDecoder.get(), static_cast<const rti1516ev::HLAinteger64Time&>(time).getTime(), orderType);
  }
}
...
void FlexRayFrameInteractionClass::ReceiveInteraction(const rti1516ev::ParameterHandleValueMap& parameters , const rti1516ev::LogicalTime& time, OrderType orderType)
{
  ...
  for (auto& entry : _receiveCallbacksWithTime) {
    auto& callback = entry.second;
    callback(IsRequestDecoder.get(), ChannelNameDecoder.get(), static_cast<BusType>(BusTypeDecoder.get()), RequestingFederateDecoder, SenderDecoder, ReceiverDecoder, FrameIDDecoder.get(), PayloadPreambleIndicatorDecoder.get(), HeaderDecoder, PayloadDecoder, static_cast<const rti1516ev::HLAinteger64Time&>(time).getTime(), orderType);
  }
}
...
void PythonCommandInteractionClass::ReceiveInteraction(const rti1516ev::ParameterHandleValueMap& parameters , const rti1516ev::LogicalTime& time, OrderType orderType)
{
  ...
  for (auto& entry : _receiveCallbacksWithTime) {
    auto& callback = entry.second;
    callback(CodeDecoder.get(), TargetDecoder.get(), RefIDDecoder.get(), static_cast<const rti1516ev::HLAinteger64Time&>(time).getTime(), orderType);
  }
}
