...
void SystemVariableObjectClass::DiscoverObjectInstance(rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  assert(mObjectInstancesByHandle.find(objectInstanceHandle) == mObjectInstancesByHandle.end());
  std::shared_ptr<SystemVariable> discoveredObject;
  auto objectInstanceIter = mObjectInstancesByName.find(objectInstanceName);
  if (objectInstanceIter == mObjectInstancesByName.end())
  {
    discoveredObject = std::shared_ptr<SystemVariable>(mCreatorFunction(this, objectInstanceName, mRtiAmbassador));
    InsertObjectInstanceName(discoveredObject, objectInstanceName);
  }
  else
  {
    // Federate currently tries to reserve an object that was already created by another federate
    discoveredObject = std::dynamic_pointer_cast<SystemVariable>(objectInstanceIter->second);
  }
  discoveredObject->mObjectInstanceHandle = objectInstanceHandle;
  discoveredObject->mIsOwner = false;
  InsertObjectInstanceHandle(discoveredObject, objectInstanceHandle);
...
void SystemVariable::ReflectAttributeValues(const rti1516ev::AttributeHandleValueMap& attributes, const rti1516ev::LogicalTime& time, OrderType orderType)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  for (auto& attributeHandleValue : attributes)
  {
...
void ValueEntityObjectClass::DiscoverObjectInstance(rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  assert(mObjectInstancesByHandle.find(objectInstanceHandle) == mObjectInstancesByHandle.end());
  std::shared_ptr<ValueEntity> discoveredObject;
  auto objectInstanceIter = mObjectInstancesByName.find(objectInstanceName);
  if (objectInstanceIter == mObjectInstancesByName.end())
  {
    discoveredObject = std::shared_ptr<ValueEntity>(mCreatorFunction(this, objectInstanceName, mRtiAmbassador));
    InsertObjectInstanceName(discoveredObject, objectInstanceName);
  }
  else
  {
    // Federate currently tries to reserve an object that was already created by another federate
    discoveredObject = std::dynamic_pointer_cast<ValueEntity>(objectInstanceIter->second);
  }
  discoveredObject->mObjectInstanceHandle = objectInstanceHandle;
  discoveredObject->mIsOwner = false;
  InsertObjectInstanceHandle(discoveredObject, objectInstanceHandle);
...
void ValueEntity::ReflectAttributeValues(const rti1516ev::AttributeHandleValueMap& attributes, const rti1516ev::LogicalTime& time, OrderType orderType)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  for (auto& attributeHandleValue : attributes)
  {
...
void DOMemberSourceObjectClass::DiscoverObjectInstance(rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  assert(mObjectInstancesByHandle.find(objectInstanceHandle) == mObjectInstancesByHandle.end());
  std::shared_ptr<DOMemberSource> discoveredObject;
  auto objectInstanceIter = mObjectInstancesByName.find(objectInstanceName);
  if (objectInstanceIter == mObjectInstancesByName.end())
  {
    discoveredObject = std::shared_ptr<DOMemberSource>(mCreatorFunction(this, objectInstanceName, mRtiAmbassador));
    InsertObjectInstanceName(discoveredObject, objectInstanceName);
  }
  else
  {
    // Federate currently tries to reserve an object that was already created by another federate
    discoveredObject = std::dynamic_pointer_cast<DOMemberSource>(objectInstanceIter->second);
  }
  discoveredObject->mObjectInstanceHandle = objectInstanceHandle;
  discoveredObject->mIsOwner = false;
  InsertObjectInstanceHandle(discoveredObject, objectInstanceHandle);
...
void DOMemberSource::ReflectAttributeValues(const rti1516ev::AttributeHandleValueMap& attributes, const rti1516ev::LogicalTime& time, OrderType orderType)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  for (auto& attributeHandleValue : attributes)
  {
...
void DOMemberTargetObjectClass::DiscoverObjectInstance(rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  assert(mObjectInstancesByHandle.find(objectInstanceHandle) == mObjectInstancesByHandle.end());
  std::shared_ptr<DOMemberTarget> discoveredObject;
  auto objectInstanceIter = mObjectInstancesByName.find(objectInstanceName);
  if (objectInstanceIter == mObjectInstancesByName.end())
  {
    discoveredObject = std::shared_ptr<DOMemberTarget>(mCreatorFunction(this, objectInstanceName, mRtiAmbassador));
    InsertObjectInstanceName(discoveredObject, objectInstanceName);
  }
  else
  {
    // Federate currently tries to reserve an object that was already created by another federate
    discoveredObject = std::dynamic_pointer_cast<DOMemberTarget>(objectInstanceIter->second);
  }
  discoveredObject->mObjectInstanceHandle = objectInstanceHandle;
  discoveredObject->mIsOwner = false;
  InsertObjectInstanceHandle(discoveredObject, objectInstanceHandle);
...
void DOMemberTarget::ReflectAttributeValues(const rti1516ev::AttributeHandleValueMap& attributes, const rti1516ev::LogicalTime& time, OrderType orderType)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  for (auto& attributeHandleValue : attributes)
  {
...
void BusManagementObjectClass::DiscoverObjectInstance(rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  assert(mObjectInstancesByHandle.find(objectInstanceHandle) == mObjectInstancesByHandle.end());
  std::shared_ptr<BusManagement> discoveredObject;
  auto objectInstanceIter = mObjectInstancesByName.find(objectInstanceName);
  if (objectInstanceIter == mObjectInstancesByName.end())
  {
    discoveredObject = std::shared_ptr<BusManagement>(mCreatorFunction(this, objectInstanceName, mRtiAmbassador));
    InsertObjectInstanceName(discoveredObject, objectInstanceName);
  }
  else
  {
    // Federate currently tries to reserve an object that was already created by another federate
    discoveredObject = std::dynamic_pointer_cast<BusManagement>(objectInstanceIter->second);
  }
  discoveredObject->mObjectInstanceHandle = objectInstanceHandle;
  discoveredObject->mIsOwner = false;
  InsertObjectInstanceHandle(discoveredObject, objectInstanceHandle);
...
void BusManagement::ReflectAttributeValues(const rti1516ev::AttributeHandleValueMap& attributes, const rti1516ev::LogicalTime& /* time */, OrderType /* orderType */)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  for (auto& attributeHandleValue : attributes)
  {
...
void BusManagementCanObjectClass::DiscoverObjectInstance(rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  assert(mObjectInstancesByHandle.find(objectInstanceHandle) == mObjectInstancesByHandle.end());
  std::shared_ptr<BusManagementCan> discoveredObject;
  auto objectInstanceIter = mObjectInstancesByName.find(objectInstanceName);
  if (objectInstanceIter == mObjectInstancesByName.end())
  {
    discoveredObject = std::shared_ptr<BusManagementCan>(mCreatorFunction(this, objectInstanceName, mRtiAmbassador));
    InsertObjectInstanceName(discoveredObject, objectInstanceName);
  }
  else
  {
    // Federate currently tries to reserve an object that was already created by another federate
    discoveredObject = std::dynamic_pointer_cast<BusManagementCan>(objectInstanceIter->second);
  }
  discoveredObject->mObjectInstanceHandle = objectInstanceHandle;
  discoveredObject->mIsOwner = false;
  InsertObjectInstanceHandle(discoveredObject, objectInstanceHandle);
...
void BusManagementCan::ReflectAttributeValues(const rti1516ev::AttributeHandleValueMap& attributes, const rti1516ev::LogicalTime& time, OrderType orderType)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  for (auto& attributeHandleValue : attributes)
  {
...
void BusManagementEthernetObjectClass::DiscoverObjectInstance(rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  assert(mObjectInstancesByHandle.find(objectInstanceHandle) == mObjectInstancesByHandle.end());
  std::shared_ptr<BusManagementEthernet> discoveredObject;
  auto objectInstanceIter = mObjectInstancesByName.find(objectInstanceName);
  if (objectInstanceIter == mObjectInstancesByName.end())
  {
    discoveredObject = std::shared_ptr<BusManagementEthernet>(mCreatorFunction(this, objectInstanceName, mRtiAmbassador));
    InsertObjectInstanceName(discoveredObject, objectInstanceName);
  }
  else
  {
    // Federate currently tries to reserve an object that was already created by another federate
    discoveredObject = std::dynamic_pointer_cast<BusManagementEthernet>(objectInstanceIter->second);
  }
  discoveredObject->mObjectInstanceHandle = objectInstanceHandle;
  discoveredObject->mIsOwner = false;
  InsertObjectInstanceHandle(discoveredObject, objectInstanceHandle);
...
void BusManagementEthernet::ReflectAttributeValues(const rti1516ev::AttributeHandleValueMap& attributes, const rti1516ev::LogicalTime& time, OrderType orderType)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  for (auto& attributeHandleValue : attributes)
  {
...
void FlexRayClusterObjectClass::DiscoverObjectInstance(rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  assert(mObjectInstancesByHandle.find(objectInstanceHandle) == mObjectInstancesByHandle.end());
  std::shared_ptr<FlexRayCluster> discoveredObject;
  auto objectInstanceIter = mObjectInstancesByName.find(objectInstanceName);
  if (objectInstanceIter == mObjectInstancesByName.end())
  {
    discoveredObject = std::shared_ptr<FlexRayCluster>(mCreatorFunction(this, objectInstanceName, mRtiAmbassador));
    InsertObjectInstanceName(discoveredObject, objectInstanceName);
  }
  else
  {
    // Federate currently tries to reserve an object that was already created by another federate
    discoveredObject = std::dynamic_pointer_cast<FlexRayCluster>(objectInstanceIter->second);
  }
  discoveredObject->mObjectInstanceHandle = objectInstanceHandle;
  discoveredObject->mIsOwner = false;
  InsertObjectInstanceHandle(discoveredObject, objectInstanceHandle);
...
void FlexRayCluster::ReflectAttributeValues(const rti1516ev::AttributeHandleValueMap& attributes, const rti1516ev::LogicalTime& time, OrderType orderType)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  for (auto& attributeHandleValue : attributes)
  {
...
void BusControllerObjectClass::DiscoverObjectInstance(rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  assert(mObjectInstancesByHandle.find(objectInstanceHandle) == mObjectInstancesByHandle.end());
  std::shared_ptr<BusController> discoveredObject;
  auto objectInstanceIter = mObjectInstancesByName.find(objectInstanceName);
  if (objectInstanceIter == mObjectInstancesByName.end())
  {
    discoveredObject = std::shared_ptr<BusController>(mCreatorFunction(this, objectInstanceName, mRtiAmbassador));
    InsertObjectInstanceName(discoveredObject, objectInstanceName);
  }
  else
  {
    // Federate currently tries to reserve an object that was already created by another federate
    discoveredObject = std::dynamic_pointer_cast<BusController>(objectInstanceIter->second);
  }
  discoveredObject->mObjectInstanceHandle = objectInstanceHandle;
  discoveredObject->mIsOwner = false;
  InsertObjectInstanceHandle(discoveredObject, objectInstanceHandle);
...
void BusController::ReflectAttributeValues(const rti1516ev::AttributeHandleValueMap& attributes, const rti1516ev::LogicalTime& /* time */, OrderType /* orderType */)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  for (auto& attributeHandleValue : attributes)
  {
...
void BusControllerCanObjectClass::DiscoverObjectInstance(rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  assert(mObjectInstancesByHandle.find(objectInstanceHandle) == mObjectInstancesByHandle.end());
  std::shared_ptr<BusControllerCan> discoveredObject;
  auto objectInstanceIter = mObjectInstancesByName.find(objectInstanceName);
  if (objectInstanceIter == mObjectInstancesByName.end())
  {
    discoveredObject = std::shared_ptr<BusControllerCan>(mCreatorFunction(this, objectInstanceName, mRtiAmbassador));
    InsertObjectInstanceName(discoveredObject, objectInstanceName);
  }
  else
  {
    // Federate currently tries to reserve an object that was already created by another federate
    discoveredObject = std::dynamic_pointer_cast<BusControllerCan>(objectInstanceIter->second);
  }
  discoveredObject->mObjectInstanceHandle = objectInstanceHandle;
  discoveredObject->mIsOwner = false;
  InsertObjectInstanceHandle(discoveredObject, objectInstanceHandle);
...
void BusControllerCan::ReflectAttributeValues(const rti1516ev::AttributeHandleValueMap& attributes, const rti1516ev::LogicalTime& time, OrderType orderType)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  for (auto& attributeHandleValue : attributes)
  {
...
void BusControllerEthernetObjectClass::DiscoverObjectInstance(rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  assert(mObjectInstancesByHandle.find(objectInstanceHandle) == mObjectInstancesByHandle.end());
  std::shared_ptr<BusControllerEthernet> discoveredObject;
  auto objectInstanceIter = mObjectInstancesByName.find(objectInstanceName);
  if (objectInstanceIter == mObjectInstancesByName.end())
  {
    discoveredObject = std::shared_ptr<BusControllerEthernet>(mCreatorFunction(this, objectInstanceName, mRtiAmbassador));
    InsertObjectInstanceName(discoveredObject, objectInstanceName);
  }
  else
  {
    // Federate currently tries to reserve an object that was already created by another federate
    discoveredObject = std::dynamic_pointer_cast<BusControllerEthernet>(objectInstanceIter->second);
  }
  discoveredObject->mObjectInstanceHandle = objectInstanceHandle;
  discoveredObject->mIsOwner = false;
  InsertObjectInstanceHandle(discoveredObject, objectInstanceHandle);
...
void BusControllerEthernet::ReflectAttributeValues(const rti1516ev::AttributeHandleValueMap& attributes, const rti1516ev::LogicalTime& time, OrderType orderType)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  for (auto& attributeHandleValue : attributes)
  {
...
void FlexRayControllerStatusObjectClass::DiscoverObjectInstance(rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  assert(mObjectInstancesByHandle.find(objectInstanceHandle) == mObjectInstancesByHandle.end());
  std::shared_ptr<FlexRayControllerStatus> discoveredObject;
  auto objectInstanceIter = mObjectInstancesByName.find(objectInstanceName);
  if (objectInstanceIter == mObjectInstancesByName.end())
  {
    discoveredObject = std::shared_ptr<FlexRayControllerStatus>(mCreatorFunction(this, objectInstanceName, mRtiAmbassador));
    InsertObjectInstanceName(discoveredObject, objectInstanceName);
  }
  else
  {
    // Federate currently tries to reserve an object that was already created by another federate
    discoveredObject = std::dynamic_pointer_cast<FlexRayControllerStatus>(objectInstanceIter->second);
  }
  discoveredObject->mObjectInstanceHandle = objectInstanceHandle;
  discoveredObject->mIsOwner = false;
  InsertObjectInstanceHandle(discoveredObject, objectInstanceHandle);
...
void FlexRayControllerStatus::ReflectAttributeValues(const rti1516ev::AttributeHandleValueMap& attributes, const rti1516ev::LogicalTime& time, OrderType orderType)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  for (auto& attributeHandleValue : attributes)
  {
...
void FlexRayControllerObjectClass::DiscoverObjectInstance(rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  assert(mObjectInstancesByHandle.find(objectInstanceHandle) == mObjectInstancesByHandle.end());
  std::shared_ptr<FlexRayController> discoveredObject;
  auto objectInstanceIter = mObjectInstancesByName.find(objectInstanceName);
  if (objectInstanceIter == mObjectInstancesByName.end())
  {
    discoveredObject = std::shared_ptr<FlexRayController>(mCreatorFunction(this, objectInstanceName, mRtiAmbassador));
    InsertObjectInstanceName(discoveredObject, objectInstanceName);
  }
  else
  {
    // Federate currently tries to reserve an object that was already created by another federate
    discoveredObject = std::dynamic_pointer_cast<FlexRayController>(objectInstanceIter->second);
  }
  discoveredObject->mObjectInstanceHandle = objectInstanceHandle;
  discoveredObject->mIsOwner = false;
  InsertObjectInstanceHandle(discoveredObject, objectInstanceHandle);
...
void FlexRayController::ReflectAttributeValues(const rti1516ev::AttributeHandleValueMap& attributes, const rti1516ev::LogicalTime& time, OrderType orderType)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  for (auto& attributeHandleValue : attributes)
  {
...
FlexRaySendBufferObjectClass::FlexRaySendBufferObjectClass(rti1516ev::RTIambassador* rtiAmbassador, ObjectClassRegistry* registry, HLAobjectRootObjectClass* baseClass)
{
  ...
  mPayloadLengthInWordsAttributeHandle = rtiAmbassador->getAttributeHandle(mObjectClassHandle, L"PayloadLengthInWords");
...
rti1516ev::AttributeHandleSet FlexRaySendBufferObjectClass::GetAllAttributeHandles()
{
  ...
  result.insert(GetPayloadLengthInWordsAttributeHandle());
  return result;
}
...
void FlexRaySendBufferObjectClass::DiscoverObjectInstance(rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  assert(mObjectInstancesByHandle.find(objectInstanceHandle) == mObjectInstancesByHandle.end());
  std::shared_ptr<FlexRaySendBuffer> discoveredObject;
  auto objectInstanceIter = mObjectInstancesByName.find(objectInstanceName);
  if (objectInstanceIter == mObjectInstancesByName.end())
  {
    discoveredObject = std::shared_ptr<FlexRaySendBuffer>(mCreatorFunction(this, objectInstanceName, mRtiAmbassador));
    InsertObjectInstanceName(discoveredObject, objectInstanceName);
  }
  else
  {
    // Federate currently tries to reserve an object that was already created by another federate
    discoveredObject = std::dynamic_pointer_cast<FlexRaySendBuffer>(objectInstanceIter->second);
  }
  discoveredObject->mObjectInstanceHandle = objectInstanceHandle;
  discoveredObject->mIsOwner = false;
  InsertObjectInstanceHandle(discoveredObject, objectInstanceHandle);
...
// attribute PayloadLengthInWords : HLAoctet
uint8_t FlexRaySendBuffer::GetPayloadLengthInWords() const
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  return mPayloadLengthInWords.get();
}

void FlexRaySendBuffer::SetPayloadLengthInWords(uint8_t newValue)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  // CanTranslateToCpp=True
  mPayloadLengthInWords.set(newValue);
  mDirty |= kPayloadLengthInWordsBit;
  mValuesSet |= kPayloadLengthInWordsBit;
}
...
rti1516ev::AttributeHandleValueMap FlexRaySendBuffer::GetAllAttributeValues() const
{
  ...
  result[mObjectClass->GetPayloadLengthInWordsAttributeHandle()] = mPayloadLengthInWords.encode();
  return result;
}
...
rti1516ev::AttributeHandleValueMap FlexRaySendBuffer::GetModifiedAttributeValues() const
{
  ...
  if (mDirty & kPayloadLengthInWordsBit)
  {
    result[mObjectClass->GetPayloadLengthInWordsAttributeHandle()] = mPayloadLengthInWords.encode();
  }
  return result;
...
void FlexRaySendBuffer::ReflectAttributeValues(const rti1516ev::AttributeHandleValueMap& attributes)
{
  ...
  for (auto& attributeHandleValue : attributes)
  {
    ...
    else if (attributeHandle == mObjectClass->GetPayloadLengthInWordsAttributeHandle())
    {
      mPayloadLengthInWords.decode(attributeHandleValue.second);
      mLastUpdated |= kPayloadLengthInWordsBit;
    }
  } // for (auto& attributeHandleValue : attributes)
  mValuesReceived |= mLastUpdated;
  ExecuteUpdateCallbacks(optional<int64_t>(), optional<OrderType>());
...
void FlexRaySendBuffer::ReflectAttributeValues(const rti1516ev::AttributeHandleValueMap& attributes, const rti1516ev::LogicalTime& time, OrderType orderType)
{
  std::lock_guard<std::recursive_mutex> lock(mMutex);
  for (auto& attributeHandleValue : attributes)
  {
...
void FlexRaySendBuffer::RequestAttributeValues()
{
  rti1516ev::AttributeHandleSet requestAttributes;
  {
    ...
    if ((mLastUpdated & kPayloadLengthInWordsBit) == 0)
    {
      requestAttributes.insert(mObjectClass->GetPayloadLengthInWordsAttributeHandle());
    }
  } // scope of lock_guard
  mRtiAmbassador->requestAttributeValueUpdate(mObjectInstanceHandle, requestAttributes, rti1516ev::VariableLengthData());
}
...
void FlexRaySendBuffer::ProvideAttributeValues(const rti1516ev::AttributeHandleSet& attributeHandles, optional<int64_t> time)
{
  rti1516ev::AttributeHandleValueMap updateAttributes;
  {
    ...
    for (auto& attributeHandle : attributeHandles)
    {
      ...
      else if (attributeHandle == mObjectClass->GetPayloadLengthInWordsAttributeHandle() && (mValuesSet & kPayloadLengthInWordsBit) != 0)
      {
        updateAttributes.insert(std::make_pair(attributeHandle, mPayloadLengthInWords.encode()));
        mDirty &= ~kPayloadLengthInWordsBit;
      }
    } // for (auto& attributeHandleValue : attributes)
  } // scope of lock_guard
...
void ObjectClassRegistry::Initialize(rti1516ev::RTIambassador* rtiAmbassador)
{
  ...
  try
  {
    ...
    mDiscoverCallbacks.insert(std::make_pair(
      mSystemVariableObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName) {
        mSystemVariableObjectClass->DiscoverObjectInstance(objectInstanceHandle, objectInstanceName);
      }));
    mRemoveCallbacks.insert(std::make_pair(
      mSystemVariableObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle) {
        mSystemVariableObjectClass->RemoveObjectInstance(objectInstanceHandle);
      }));
    mReflectCallbacks.insert(std::make_pair(
      mSystemVariableObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle theObject, const rti1516ev::AttributeHandleValueMap & attributes) {
  ...
  try
  {
    ...
    mDiscoverCallbacks.insert(std::make_pair(
      mValueEntityObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName) {
        mValueEntityObjectClass->DiscoverObjectInstance(objectInstanceHandle, objectInstanceName);
      }));
    mRemoveCallbacks.insert(std::make_pair(
      mValueEntityObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle) {
        mValueEntityObjectClass->RemoveObjectInstance(objectInstanceHandle);
      }));
    mReflectCallbacks.insert(std::make_pair(
      mValueEntityObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle theObject, const rti1516ev::AttributeHandleValueMap & attributes) {
  ...
  try
  {
    ...
    mDiscoverCallbacks.insert(std::make_pair(
      mDOMemberSourceObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName) {
        mDOMemberSourceObjectClass->DiscoverObjectInstance(objectInstanceHandle, objectInstanceName);
      }));
    mRemoveCallbacks.insert(std::make_pair(
      mDOMemberSourceObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle) {
        mDOMemberSourceObjectClass->RemoveObjectInstance(objectInstanceHandle);
      }));
    mReflectCallbacks.insert(std::make_pair(
      mDOMemberSourceObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle theObject, const rti1516ev::AttributeHandleValueMap & attributes) {
  ...
  try
  {
    ...
    mDiscoverCallbacks.insert(std::make_pair(
      mDOMemberTargetObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName) {
        mDOMemberTargetObjectClass->DiscoverObjectInstance(objectInstanceHandle, objectInstanceName);
      }));
    mRemoveCallbacks.insert(std::make_pair(
      mDOMemberTargetObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle) {
        mDOMemberTargetObjectClass->RemoveObjectInstance(objectInstanceHandle);
      }));
    mReflectCallbacks.insert(std::make_pair(
      mDOMemberTargetObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle theObject, const rti1516ev::AttributeHandleValueMap & attributes) {
  ...
  try
  {
    ...
    mDiscoverCallbacks.insert(std::make_pair(
      mBusManagementObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName) {
        mBusManagementObjectClass->DiscoverObjectInstance(objectInstanceHandle, objectInstanceName);
      }));
    mRemoveCallbacks.insert(std::make_pair(
      mBusManagementObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle) {
        mBusManagementObjectClass->RemoveObjectInstance(objectInstanceHandle);
      }));
    mReflectCallbacks.insert(std::make_pair(
      mBusManagementObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle theObject, const rti1516ev::AttributeHandleValueMap & attributes) {
  ...
  try
  {
    ...
    mDiscoverCallbacks.insert(std::make_pair(
      mBusManagementCanObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName) {
        mBusManagementCanObjectClass->DiscoverObjectInstance(objectInstanceHandle, objectInstanceName);
      }));
    mRemoveCallbacks.insert(std::make_pair(
      mBusManagementCanObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle) {
        mBusManagementCanObjectClass->RemoveObjectInstance(objectInstanceHandle);
      }));
    mReflectCallbacks.insert(std::make_pair(
      mBusManagementCanObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle theObject, const rti1516ev::AttributeHandleValueMap & attributes) {
  ...
  try
  {
    ...
    mDiscoverCallbacks.insert(std::make_pair(
      mBusManagementEthernetObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName) {
        mBusManagementEthernetObjectClass->DiscoverObjectInstance(objectInstanceHandle, objectInstanceName);
      }));
    mRemoveCallbacks.insert(std::make_pair(
      mBusManagementEthernetObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle) {
        mBusManagementEthernetObjectClass->RemoveObjectInstance(objectInstanceHandle);
      }));
    mReflectCallbacks.insert(std::make_pair(
      mBusManagementEthernetObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle theObject, const rti1516ev::AttributeHandleValueMap & attributes) {
  ...
  try
  {
    ...
    mDiscoverCallbacks.insert(std::make_pair(
      mFlexRayClusterObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName) {
        mFlexRayClusterObjectClass->DiscoverObjectInstance(objectInstanceHandle, objectInstanceName);
      }));
    mRemoveCallbacks.insert(std::make_pair(
      mFlexRayClusterObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle) {
        mFlexRayClusterObjectClass->RemoveObjectInstance(objectInstanceHandle);
      }));
    mReflectCallbacks.insert(std::make_pair(
      mFlexRayClusterObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle theObject, const rti1516ev::AttributeHandleValueMap & attributes) {
  ...
  try
  {
    ...
    mDiscoverCallbacks.insert(std::make_pair(
      mBusControllerObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName) {
        mBusControllerObjectClass->DiscoverObjectInstance(objectInstanceHandle, objectInstanceName);
      }));
    mRemoveCallbacks.insert(std::make_pair(
      mBusControllerObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle) {
        mBusControllerObjectClass->RemoveObjectInstance(objectInstanceHandle);
      }));
    mReflectCallbacks.insert(std::make_pair(
      mBusControllerObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle theObject, const rti1516ev::AttributeHandleValueMap & attributes) {
  ...
  try
  {
    ...
    mDiscoverCallbacks.insert(std::make_pair(
      mBusControllerCanObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName) {
        mBusControllerCanObjectClass->DiscoverObjectInstance(objectInstanceHandle, objectInstanceName);
      }));
    mRemoveCallbacks.insert(std::make_pair(
      mBusControllerCanObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle) {
        mBusControllerCanObjectClass->RemoveObjectInstance(objectInstanceHandle);
      }));
    mReflectCallbacks.insert(std::make_pair(
      mBusControllerCanObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle theObject, const rti1516ev::AttributeHandleValueMap & attributes) {
  ...
  try
  {
    ...
    mDiscoverCallbacks.insert(std::make_pair(
      mBusControllerEthernetObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName) {
        mBusControllerEthernetObjectClass->DiscoverObjectInstance(objectInstanceHandle, objectInstanceName);
      }));
    mRemoveCallbacks.insert(std::make_pair(
      mBusControllerEthernetObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle) {
        mBusControllerEthernetObjectClass->RemoveObjectInstance(objectInstanceHandle);
      }));
    mReflectCallbacks.insert(std::make_pair(
      mBusControllerEthernetObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle theObject, const rti1516ev::AttributeHandleValueMap & attributes) {
  ...
  try
  {
    ...
    mDiscoverCallbacks.insert(std::make_pair(
      mFlexRayControllerStatusObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName) {
        mFlexRayControllerStatusObjectClass->DiscoverObjectInstance(objectInstanceHandle, objectInstanceName);
      }));
    mRemoveCallbacks.insert(std::make_pair(
      mFlexRayControllerStatusObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle) {
        mFlexRayControllerStatusObjectClass->RemoveObjectInstance(objectInstanceHandle);
      }));
    mReflectCallbacks.insert(std::make_pair(
      mFlexRayControllerStatusObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle theObject, const rti1516ev::AttributeHandleValueMap & attributes) {
  ...
  try
  {
    ...
    mDiscoverCallbacks.insert(std::make_pair(
      mFlexRayControllerObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName) {
        mFlexRayControllerObjectClass->DiscoverObjectInstance(objectInstanceHandle, objectInstanceName);
      }));
    mRemoveCallbacks.insert(std::make_pair(
      mFlexRayControllerObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle) {
        mFlexRayControllerObjectClass->RemoveObjectInstance(objectInstanceHandle);
      }));
    mReflectCallbacks.insert(std::make_pair(
      mFlexRayControllerObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle theObject, const rti1516ev::AttributeHandleValueMap & attributes) {
  ...
  try
  {
    ...
    mDiscoverCallbacks.insert(std::make_pair(
      mFlexRaySendBufferObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle, const std::wstring& objectInstanceName) {
        mFlexRaySendBufferObjectClass->DiscoverObjectInstance(objectInstanceHandle, objectInstanceName);
      }));
    mRemoveCallbacks.insert(std::make_pair(
      mFlexRaySendBufferObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle objectInstanceHandle) {
        mFlexRaySendBufferObjectClass->RemoveObjectInstance(objectInstanceHandle);
      }));
    mReflectCallbacks.insert(std::make_pair(
      mFlexRaySendBufferObjectClass->GetObjectClassHandle(),
      [this](rti1516ev::ObjectInstanceHandle theObject, const rti1516ev::AttributeHandleValueMap & attributes) {
