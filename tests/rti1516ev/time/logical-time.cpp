...
main(int argc, char* argv[])
{
  ...
  std::unique_ptr<rti1516ev::LogicalTimeFactory> factory;
  factory = rti1516ev::LogicalTimeFactoryFactory::makeLogicalTimeFactory(OpenRTI::localeToUcs(implementationName));
  if (!factory.get()) {
    std::cerr << "Can not create logical time factory: \"" << implementationName << "\"!" << std::endl;
    return EXIT_FAILURE;
  }

  std::unique_ptr<rti1516ev::LogicalTimeInterval> logicalTimeInterval = factory->makeZero();
  ...
  std::unique_ptr<rti1516ev::LogicalTime> logicalTime = factory->makeInitial();
  if (!logicalTime.get()) {
  ...
  std::unique_ptr<rti1516ev::LogicalTime> initialTime = factory->makeInitial();
  if (!initialTime.get()) {
    std::cerr << "Can not create logical time: \"" << implementationName << "\"!" << std::endl;
    return EXIT_FAILURE;
  }

  std::unique_ptr<rti1516ev::LogicalTime> finalTime = factory->makeFinal();
  if (!finalTime.get()) {
    std::cerr << "Can not create logical time: \"" << implementationName << "\"!" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  std::unique_ptr<rti1516ev::LogicalTime> logicalTime2 = factory->makeInitial();
  if (!logicalTime2.get()) {
    std::cerr << "Can not create logical time: \"" << implementationName << "\"!" << std::endl;
    return EXIT_FAILURE;
  }
  std::unique_ptr<rti1516ev::LogicalTimeInterval> logicalTimeInterval2 = factory->makeZero();
