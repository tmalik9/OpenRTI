...
public:
  ...
  virtual bool exec(uint32_t threadIndex) override
  {
    ...
    for (unsigned i = 0; i < n; ++i) {
      ...
      } catch (const rti1516ev::InconsistentFDD& e) {
        if (expectCreateSuccess && getDisjointFederations()) {
          std::wcout << L"createFederationExecution does not succeed as required!" << std::endl;
          std::wcout << L"rti1516ev::InconsistentFDD: \"" << e.what() << L"\"" << std::endl;
          return false;
        }
      } catch (const rti1516ev::FederationExecutionAlreadyExists& e) {
        if (getDisjointFederations()) {
          std::wcout << L"createFederationExecution does not succeed as required!" << std::endl;
          std::wcout << L"rti1516ev::FederationExecutionAlreadyExists: \"" << e.what() << L"\"" << std::endl;
          return false;
        }
      ...
      for (unsigned j = 0; j < 10; ++j) {
        ...
        } catch (const rti1516ev::FederationExecutionDoesNotExist& e) {
          if (expectCreateSuccess && getDisjointFederations()) {
            std::wcout << L"joinFederationExecution does not succeed as required!" << std::endl;
            std::wcout << L"rti1516ev::FederationExecutionDoesNotExist: \"" << e.what() << L"\"" << std::endl;
            return false;
          }
        } catch (const rti1516ev::InconsistentFDD& e) {
          ...
          for (std::map<std::wstring, InteractionClass>::const_iterator i = allModule._interactionClasses.begin();
            ...
            try {
              ...
              if (expectedAvailable) {
                ...
                std::wcout << L"rti1516ev::NameNotFound: \"" << e.what() << L"\"" << std::endl;
                return false;
              }
            } catch (const rti1516ev::Exception& e) {
              std::wcout << L"rti1516ev::Exception: \"" << e.what() << L"\"" << std::endl;
        ...
        } catch (const rti1516ev::FederateNotExecutionMember& e) {
          if (successfullyJoined) {
            std::wcout << L"resignFederationExecution does not succeed as required!" << std::endl;
            std::wcout << L"rti1516ev::FederateNotExecutionMember: \"" << e.what() << L"\"" << std::endl;
            return false;
          }
        } catch (const rti1516ev::Exception& e) {
          std::wcout << L"rti1516ev::Exception: \"" << e.what() << L"\"" << std::endl;
      ...
      } catch (const rti1516ev::FederatesCurrentlyJoined& e) {
        if (successfullyCreated && getDisjointFederations()) {
          std::wcout << L"destroyFederationExecution does not succeed as required!" << std::endl;
          std::wcout << L"rti1516ev::FederatesCurrentlyJoined: \"" << e.what() << L"\"" << std::endl;
          return false;
        }
      } catch (const rti1516ev::Exception& e) {
        std::wcout << L"rti1516ev::Exception: \"" << e.what() << L"\"" << std::endl;
