 ...
 */
std::wstring
TimeRegulatingFederate::convertStringToWstring(const std::string& str)
{
 ...
 * @return void
 */
void TimeRegulatingFederate::runFederate(std::string federateName, std::string fom, std::string address)
{
  ///
  ...
  this->fedamb = new TimeRegulatingFedAmb(this->rtiamb.get());
 ...
 */
void TimeRegulatingFederate::initializeHandles()
{
 ...
 */
void TimeRegulatingFederate::waitForUser()
{
...
void TimeRegulatingFederate::publishAndSubscribe()
{
  ...
  myPublishedObjectClass = this->aHandle;
...
TimeRegulatingFederate::toVariableLengthData(const wchar_t* s)
{
 ...
 * has been received.
 */
void TimeRegulatingFederate::advanceTime(double timestep)
{
  wprintf(L"%s: timestep=%f\n",  __FUNCTIONW__, timestep);
  /// request the advance
  ...
  wprintf(L"%s: new time=%f\n",  __FUNCTIONW__, fedamb->federateTime);
}
