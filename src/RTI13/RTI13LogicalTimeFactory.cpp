...
public:
  ...
  std::unique_ptr<RTI::FedTime> _fedTime;
};
...
RTI13LogicalTimeFactory::nextAfter(const RTI13LogicalTimeFactory::LogicalTime& logicalTime)
{
  ...
  std::unique_ptr<RTI::FedTime> epsilonTime(RTI::FedTimeFactory::makeZero());
...
{
  std::unique_ptr<RTI::FedTime> logicalTime(RTI::FedTimeFactory::makeZero());
  *logicalTime = fedTime;
