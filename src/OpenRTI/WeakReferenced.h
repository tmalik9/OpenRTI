...
class OPENRTI_API WeakReferenced {
  public:
    ...
    static void incRef(const WeakReferenced* ref) {
      if (ref)
        ref->mWeakData->weakReferencedGet();
    }
    static void getFirst(const WeakReferenced* ref) {
      if (ref)
        ref->mWeakData->weakReferencedGetFirst();
    }
    static unsigned decRef(const WeakReferenced* ref) {
      if (ref)
        return ref->mWeakData->weakReferencedPut();
      else
        return ~0u;
    }
    static void release(const WeakReferenced* ref) {
      if (ref)
        ref->mWeakData->weakReferencedRelease();
    }
    static unsigned count(const WeakReferenced* ref) {
      if (ref)
        return ref->mWeakData->weakReferencedCount();
      else
        return ~0u;
    }

    template<typename T>
    static void destruct(T* ref) noexcept { delete ref; }
  ...
  private:
    ...
    class OPENRTI_API WeakData : public Referenced {
      public:
        ...
        void weakReferencedGet() { mRefcount.incFetch(Atomic::MemoryOrderRelease); }
        void weakReferencedGetFirst();

        unsigned weakReferencedPut() { return mRefcount.decFetch(Atomic::MemoryOrderAcqRel); }
        void weakReferencedRelease();
        unsigned weakReferencedCount() { return mRefcount & (~lastbit()); }

        void clear() noexcept { mWeakReferenced = 0; }

        WeakReferenced* getWeakReferenced();

        static unsigned lastbit() { return ~((~unsigned(0)) >> 1); }
