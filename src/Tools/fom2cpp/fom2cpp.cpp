...
int main(int argc, char** argv)
{
  ...
  if (argc == 4)
  {
    ...
    if (str_list.addFile(argv[1]))
    {
      ...
      {
        std::cerr << "Invalid variable name, must start with a letter and only contain digits and alpha characters." << std::endl;
        exit(EXIT_FAILURE);
      }
    ...
    {
      std::cerr << "Error could not load list from text file: " << argv[1] << std::endl;
      exit(EXIT_FAILURE);
    }
  }
  else if (argc == 2)
  {
    ...
    if (str_list.addStream(std::cin))
    {
      ...
      {
        std::cout << "Failed to output stream..." << std::endl;
      }
    }
  }
  return 0;
