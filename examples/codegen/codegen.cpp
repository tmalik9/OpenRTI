...
#include "implementation/SampleFDDObjects.h"
#include "implementation/SampleFDDInteractions.h"
...
bool testMomEncodings()
{
  // NOTE: a HLAtoken is equivalent to an empty byte sequence, to the encoded data will result into an empty VariableLengthArray.
  // Also, decoding will always succeed.
  OpenRTI::Mom::HLAtokenEncoding tokenEncoding;
  rti1516ev::VariableLengthData encodedData = tokenEncoding.encode();
  std::cout << "encodedData(HLAtoken)=" << encodedData << std::endl;
  OpenRTI::Mom::HLAtokenEncoding tokenDecoder;
  tokenDecoder.decode(encodedData);
  return true;
}

class OPENRTI_LOCAL SampleFDDAmbassador : public OpenRTI::RTI1516ESimpleAmbassador
{
public:
  SampleFDDAmbassador() 
  ...
  NDistSimIB::NSampleFDDEncoding::ObjectClassRegistry objectClassRegistry;
  NDistSimIB::NSampleFDDEncoding::InteractionClassRegistry interactionClassRegistry;
};


using NDistSimIB::NSampleFDDEncoding::optional;
using NDistSimIB::NSampleFDDEncoding::nullopt;
bool testSampleFDDFederate(int argc, char* argv[])
{
  OpenRTI::Options options(argc, argv);
  std::wstring fomModule = L"SampleFDD.xml";
  ...
  SampleFDDAmbassador ambassador;
  ...
  auto* measurementInitInteractionClass = ambassador.interactionClassRegistry.GetStartInteractionClass();
  measurementInitInteractionClass->RegisterReceiveCallback([&received](optional<int64_t> time, optional<NDistSimIB::NSampleFDDEncoding::OrderType> orderType)
  {
...
int main(int argc, char* argv[])
{
  ...
  if (!testHLAobjectClassBasedCountsEncoding())
    return EXIT_FAILURE;
  if (!testSampleFDDFederate(argc, argv))
    return EXIT_FAILURE;
