...
Mom::MomTreeNodeBuilder& Mom::MomTreeNodeBuilder::object(std::wstring name)
{
  try
  {
    ...
    if (!mContextStack.empty())
      ...
      fqName = name;
    rti1516ev::ObjectClassHandle handle = mRtiAmb->getObjectClassHandle(fqName);
    if (!handle.isValid())
...
Mom::MomTreeNodeBuilder& Mom::MomTreeNodeBuilder::attribute(std::wstring name, std::wstring datatype)
{
  ...
  try
  {
    rti1516ev::AttributeHandle handle = mRtiAmb->getAttributeHandle(mContextStack.front()->objectClassHandle, name);
...
Mom::MomTreeNodeBuilder& Mom::MomTreeNodeBuilder::interaction(std::wstring name)
{
  try
  {
    ...
    if (!mContextStack.empty())
      ...
      fqName = name;
    rti1516ev::InteractionClassHandle handle = mRtiAmb->getInteractionClassHandle(name);
    if (!handle.isValid())
      throw NameNotFound(name.c_str());
...
Mom::MomTreeNodeBuilder& Mom::MomTreeNodeBuilder::parameter(std::wstring name, std::wstring datatype)
{
  ...
  try
  {
    ...
    rti1516ev::ParameterHandle handle = mRtiAmb->getParameterHandle(mContextStack.front()->interactionClassHandle, name);
